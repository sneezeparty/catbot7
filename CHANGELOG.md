...
- **`/jobs` now shields your Cat Mafia (catnip) level from decay for 24 hours.** Committing a job within the last 24h suppresses both timer-based level loss (lapsed bounty) and respect-driven level loss — your mafia rank is safe as long as you do at least one job per day. Go 24h+ without any `/jobs` activity and decay resumes as before. The grace period does **not** keep catnip perks active or substitute for completing bounties; it only protects your current level. `/catnip` now shows your protection status: either "Mafia level protected by a recent job — safe until \<time\>" or "Do a `/jobs` to protect your level." Backed by `profile.last_job_time` (migration 027) and the `catnip_job_grace_hours` tuning key (default 24).
  > _draft_
- **`/vote` slash command.** Shows your last-vote time, next-vote availability (or a "You can vote now!" indicator), daily catch streak with progression preview, XP reward for this vote (doubled on weekends), and your all-time vote count. Includes a **Vote on top.gg** link button, a reminder toggle (enables/disables DM reminders), and a Refresh button to check after voting. XP is claimed separately via `/battlepass`. Requires `top_gg_modern_token` env var; disable with `voting_enabled=0`.
  > _draft_
- **Vote button on catch messages.** When voting is active and the catching player can currently vote (12h cooldown elapsed), a top.gg vote button is appended to the catch message. Drawn from the existing 20-string `vote_button_texts` pool. Appears alongside the dark-market button when both conditions apply. Disable with `voting_enabled=0`.
  > _draft_
- **`.env` file support for operators.** `config.py` now reads a gitignored `.env` file in the project root before loading any env vars. Lines follow the standard `KEY=value` format; comments (`#`) and blank lines are skipped; quoted values are unquoted. Shell-exported variables always take precedence (`os.environ.setdefault`), so one-off overrides like `voting_enabled=0 python bot.py` still work. A `.env.example` template documenting all supported variables is included in the repo. No new dependency; no behavior change for existing shell-export workflows.
  > _draft_

### Changed
- **"Spend X+ coins on a single Cat Store purchase" daily quest threshold halved: 5,000 → 2,500 coins.** The `store_spree` quest is now easier to hit on a normal shopping run. XP range (320–400) is unchanged.
  > _draft_
- **Vote XP now auto-credits across every server profile at vote time.** Previously, voting on top.gg required running `/battlepass` in each server separately to claim XP. Now, when a vote registers (webhook or polling replay), `do_vote()` iterates every `Profile` for that Discord user and calls `progress("vote")` for each — XP lands silently in the background (no in-channel level-up ceremony; that surfaces next time `/battlepass` is opened). Idempotent via the existing `vote_cooldown` check. The vote DM confirmation is updated to reflect this: "Your battlepass XP has been credited in every server you have a profile in."
  > _draft_
- **Battlepass XP requirements roughly doubled — leveling is now better paced for a 30-day season.** Per-level XP costs are ~2× higher through the front half and steepen further toward the back. Reaching Level 30 now takes ~44,500 XP (was ~22,250); Level 40 takes ~82,500 XP (was ~36,750). The post-Level-40 **Extra Rewards** grind cost rises from 1,500 → 6,000 XP per Stone pack to stay consistent with the steeper curve. Design target: an average daily player finishes around L30 in a 30-day season; a very engaged player reaches ~L40. The change affects Seasons 2–17; Season 1 is unaffected.
  > _draft_
- **Battlepass per-level XP bumped a further ~5% (Seasons 2–17) to offset vote XP re-introduction.** Example breakpoints: L1–3 1100 → 1150 XP, L28–30 2000 → 2050 XP, L31 2400 → 2500 XP, L40 6000 → 6300 XP. Cumulative cost to L30 rises 46,500 → 48,000 XP; to L40 rises 88,500 → 92,100 XP. Season 1 is untouched.
  > _draft_
- **Season 2 L1–30 curve flattened so returning veterans can actually finish the pass.** The L1–30 ramp now goes from 900 XP/level to 1,800 XP/level (blocks of 3: 900, 1000, 1100, 1200, 1300, 1400, 1500, 1600, 1700, 1800) instead of 1,150 → 2,050. **Cumulative cost to L30 drops 48,000 → 40,500 XP (−16%); L40 drops 92,100 → 84,600 XP (−8%, all from the L1–30 reduction — L31–40 untouched).** Simulation under daily-quest + first-catch + 10-streak + catnip-drip + vote XP: a *returning* engaged voter (no early-ach front-load) now hits L30 at day ~26 of a 30-day season (was day ~31 — they were missing L30 by one day after doing everything right); a *fresh* engaged voter hits L30 at ~day 19; a casual 3/4-quest player still misses L30 at ~day 33. L40 stays a 2-season stretch goal for everyone but Tier-4 perks grinders. Seasons 3–17 left on the 1,150 → 2,050 curve for now.
  > _draft_
- **Vote quest XP trimmed: 250–350 → 150–220 (weekend 2× still applies).** Voting is still rewarding but now grants less than the other four daily quests, removing any pressure to vote daily.
  > _draft_
...
- **`/catslots` now counts as its own game for the "Play 3 different casino games" extra-slot quest.** Previously a `/catslots` spin was tracked under the same `slots` bit as `/slots`, so playing both games only registered as one. `/catslots` now has its own `CASINO_GAME_BITS` entry (bit 16). The quest title is updated to list `/catslots` alongside `/slots`, `/roulette`, `/pig`, and `/cookieclicker`. Players who have already completed this quest are unaffected; players still working toward it now get credit for each game independently.
  > _draft_
- **`voting_enabled` env var default flipped: voting is now ON unless explicitly disabled.** Previously the default was `"0"` (voting off; operators had to opt in). The default is now `"1"` — `/vote`, the catch-message vote button, the battlepass vote quest, and the top.gg webhook/replay loop all activate automatically. Set `voting_enabled=0` in your environment to disable them. Token requirements are unchanged: `top_gg_modern_token` is needed for vote polling/replay, `webhook_verify` for the port-8069 webhook route.
  > _draft_
- **Mafia bounty difficulty smoothed for Lv5–Lv9.** `bounty_difficulty` targets reduced: Lv5 Consigliere 15 → 10, Lv6 Underboss 25 → 15, Lv7 Boss 50 → 28, Lv8 Godfather 70 → 40, Lv9 Kingpin 150 → 70. Because the rarity-bounty algorithm scales numeric targets directly off `bounty_difficulty`, affected bounties (e.g. "Catch 13 Rare+/5 Epic+") drop proportionally to roughly "Catch 5–6 Rare+/2–3 Epic+". Lv1–Lv4 unchanged; Lv10 El Patrón (400) and Lv11 Most Wanted (2000) unchanged — those are intentional difficulty traps.
  > _draft_
...
- **`/jobs` "no offers" message now distinguishes between a cleared board and a reputation block.** Previously the empty-board response always said "the family doesn't want anything to do with you right now — lay low, fix your reputation, and check back later," which blamed reputation even when the player had simply worked through every job in the current 12h window. Two cases are now handled separately: if eligible NPCs exist (the common case), the message says the shift board is clear and shows a Discord relative timestamp for when the next batch of jobs arrives; if no NPC will hire the player because faction reputation is below the refuse threshold with everyone, the message explains their standing is the issue. Players who previously saw the generic blame message after a productive shift will now see the accurate, actionable one.
  > _draft_
- **Vote button on catch messages now appears roughly 1-in-40 catches** (was every eligible catch). The existing 12h cooldown check is unchanged; on top of that, the button only surfaces when `random.randint(1, 40) == 1`. Reduces button fatigue for fast-catching players without removing the nudge entirely.
  > _draft_
- **`/battlepass` vote quest line is now a clickable link.** The "Vote on Top.gg" text in the unclaimed vote-quest section of the `/battlepass` embed is now a markdown link pointing directly at the top.gg vote URL, so players can click straight through without copy-pasting.
  > _draft_
- **Vote battlepass slot now shows a misc quest ~2/3 of the time.** `generate_quest("vote")` rolls 1-in-3 for the real "Vote on Top.gg" quest and 2-in-3 for a random single-action quest drawn from the misc pool (progress=1 quests, excluding the player's current misc slot and any disabled quests). The substitute reuses the existing `vote_reward` and `vote_cooldown` fields for XP and claim time; the new `profile.vote_quest` column (text, default `''`) records which misc quest is active — empty means the real vote quest. Display, claim flow, and the `do_vote` auto-grant all respect the substitute state: the vote quest line in `/battlepass` shows the substitute's title and emoji; voting does not claim a substitute (the player progresses it through its normal trigger instead). Requires migration `028_vote_substitute_slot.py` — the bot will fall back gracefully on an un-migrated DB but operators should run it before deploying.
  > _draft_

### Fixed
- **Extra Rewards Stone pack XP was split between two wrong values — now unified to 6,300.** The grant code (`grant_achievement_xp`, `progress`) had already been updated to 6,000 XP per Stone pack during the earlier rebalance, but the display/total-XP-conversion code (`_battlepass_level_info`, `gen_stats`) still read 1,500 — a leftover from before that rebalance. Post-L40 players saw `X / 1,500 XP` in `/battlepass` even though their level-ups were gating on 6,000. Both sides are now unified to 6,300 XP (matching the ~5% curve bump applied this cycle). No XP that was granted changes retroactively; only the displayed cap and overflow-level calculation are corrected.
  > _draft_
...
- **Cookie button no longer crashes with `KeyError` for users with an active casino quest.** The `/cookie` click handler was doing a partial DB fetch for `cookies` only, but `progress_casino_quest` reads `extra_quest`, `extra_cooldown`, and `casino_progress_temp` before its refetch. Any player with the casino extra-slot quest active would get a silent error instead of their click counting. The three columns are now included in the partial fetch.
  > _draft_
- **`/jobs` Overtime perk (and `reroll_board`) no longer rolls for sub-Lv4 players.** At mafia level 1–3 the player only ever sees the single tutorial errand per 12h window, so `daily_cap_extension` (Overtime) and `reroll_board` would silently expire without being usable — Overtime extends a commit cap that doesn't exist yet, and `reroll_board` reshuffles a board the player can't reach. Both are now excluded from the Lucian Jr perk-drop pool for sub-Lv4 players. They still receive the other Lucian Jr drops (`pack_drop_boost`, `free_pack`). The perks continue to roll normally from Level 4 onward.
  > _draft_
- **`/settings` no longer crashes for server admins.** The settings view was building 45 Components V2 children in a single `LayoutView`, exceeding Discord's 40-child cap and throwing an error on every open. The view is now split into two pages — **Display & cleanup** (6 toggles) and **Game features & lifecycle** (4 toggles) — with Prev/Next navigation buttons. All settings remain accessible; no toggle has been removed or renamed.
  > _draft_
...
- Migration `023_job_reroll_counter.py` adds `profile.job_rerolls_window INTEGER DEFAULT 0` and `profile.job_rerolls_window_idx BIGINT DEFAULT 0`. Idempotent per-column gate. No backfill needed. Mirrored in `schema.sql`. New `jobs.json` tuning keys: `reroll_price_per_level` (500) and `reroll_price_min` (1000). The `_jobs_do_reroll` helper is extracted from the existing perk handler and shared by the paid reroll paths (`on_reroll_paid` in `/jobs`, `on_buy_jobs_reroll` in `/catstore`).
  > _draft_
- `OWNER_ID` now reads from a new `owner_id` env var (`config.py`) with the operator's Discord ID as the fallback default. The upstream author's hardcoded ID is removed from `main.py`. The module-level `OWNER_ID` constant is initialized from `config.OWNER_ID` at load time and still overridden by `on_ready()` with the Discord application owner.
  > _draft_
- `_PERKS_REQUIRE_FULL_BOARD` frozenset introduced in `main.py` to centralize the list of perk IDs that require an unlocked multi-slot board (`daily_cap_extension`, `reroll_board`). Used by `_perks_roll_drop` to filter the pool for sub-Lv4 players.
  > _draft_
