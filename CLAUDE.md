...
- `voting_enabled` — top.gg voting, **on by default**; set `"0"` to disable. While `0`, `/vote`, the top.gg webhook route, vote-replay/reminder loops, and the catch-message vote button are all skipped. When enabled, votes flow through `do_vote()` (webhook or polling-replay loop), users claim 250–350 XP (2x weekends) via the existing `/battlepass` "Vote on Top.gg" daily quest, and `user.daily_catch_streak` advances with each vote (scaling the Loyalty Streak catnip perk and streak-pack milestones). `top_gg_modern_token` is required for polling-replay; `webhook_verify` is only needed if port 8069 is reachable from top.gg.
...
A second aiohttp server bound to **`192.168.1.155:9445`** (the host's LAN IP — single-interface bind, distinct from the public top.gg webhook server on `0.0.0.0:8069`). LAN-reachable and **unauthenticated** — never change the bind to `0.0.0.0` (broader exposure), and never grow a third write surface beyond News and Announcements. It is **primarily a read-only activity dashboard**, with **two sanctioned write surfaces** for the operator: the **News editor** (`webui/routes/news.py`, edits `config/news.json` — the data source for `/news`) and the **Announcements broadcaster** (`webui/routes/announce.py` + `webui/announce_sender.py`, writes the `announcement` table and posts a Discord-markdown body to every setupped catch channel via `ch_obj.send()` on the bot's asyncio loop). Every other section is GET-only and never mutates game state or configs. (It used to be a full editor; the broader editing surface — config-editor sections, DB toggles/field edits, `validators.py`, the Reload Bot action — was removed.) Mounted from `bot.py` setup_hook so it survives `cat!restart`.

Sections are three groups: **Insights** (Overview `/`, Activity `/activity`, Economy `/economy`, Leaderboards `/leaderboards`, Commands `/commands`), **Database** read-only browsers (Servers, Channels, Profiles, Users, Prisms, Orders under `/db/*`), and **Manage** (the two editable sections — News `/news` and Announcements `/announce`). Charts are client-side Chart.js (loaded in `base.html`); the visual system lives in `webui/static/app.css`.
