...
| Achievements | None | "Powers of Two" is an 18-achievement ladder (`pow2_8` .. `pow2_1048576`) that unlocks as a single cat rarity's count climbs past each threshold. Non-retroactive by design: a per-profile baseline (`profile.cat_milestone_base`) freezes every rarity's count at rollout (migration `040`, or on first sight for profiles it misses), so only ground covered afterward counts — a veteran sitting on 3,000 Fine skips every rung up to 2048 and earns 4096 the normal way. Exempted from the finale achievement gate alongside Hidden. |
