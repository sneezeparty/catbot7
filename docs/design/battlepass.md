...
- **All pack inventories**: every `pack_*` column (Wooden through Celestial + event packs), plus held **Mystery boxes** (`profile.mystery_boxes`) — pack-adjacent value wiped by the same `_wipe_packs()` helper
...
Each level has a configured reward: cats, packs, rain minutes, a **Scratchcard**, or a **Mystery**. As of the July 2026 hold-then-open rework, a Mystery reward no longer resolves at grant time — it's banked as a **held box** (`profile.mystery_boxes` increments by 1, shown as "You got a Mystery box! Open it in /packs") and only resolves into its contents via `resolve_mystery()` when the player opens it from the green "Open Mystery (N)" button in `/packs` (see [What a Mystery resolves to](#what-a-mystery-resolves-to), below). Held boxes are pack-adjacent value: they wipe alongside pack inventories at season rollover (`_wipe_packs()`) and are excluded from pack "Open all" by construction — each click opens exactly one box. Past the final level, the ladder enters an "Extra Rewards" tier: every 3,000 XP grants one more held Mystery box, indefinitely, via the same mechanism. (Tunable via `config/tuning.json → extra_level_xp` / `extra_level_reward`; previously a flat Stone pack every 6,300 XP, before the overflow-reward retune.) Code that reads level count uses `len(config.battle["seasons"][str(user.season)])` everywhere — adding or trimming levels per season is purely a JSON change.
...
**Design intent:** the reward curve is *front-loaded with variety* (early levels mix cat tiers and packs) and *back-loaded with lottery* — as of July 2026, levels 31–39 in every 40-level season are **Mysteries** (9 lottery pulls, now banked as held boxes the player opens at their own pace in `/packs` rather than resolved automatically), and level 40 is the one *guaranteed* per-season capstone (a Celestial pack; S3's also carries +1 rain minute). This trades guaranteed tail value for variance on purpose: the fixed Bronze→Diamond ramp it replaced was worth more in expected pack value, but every level was a foregone conclusion — the Mystery tail makes each late level a slot-machine moment, the per-level `grant_bonus_pack` drop still guarantees something every level, and the level-40 Celestial anchors the season goal. The Mystery-forever Extra Rewards tier (every 3,000 XP; previously a flat Stone per 6,300) continues past level 40 so the very-engaged never hit a wall.

### What a Mystery resolves to

**As of the July 2026 hold-then-open rework, this section describes what happens at *open* time, not grant time.** A Mystery reward is granted as a held box (`profile.mystery_boxes` += 1); nothing below rolls until the player clicks "Open Mystery" in `/packs`, which opens exactly one held box per click.

`resolve_mystery()` (main.py — called once per open, from the `/packs` "Open Mystery" button, regardless of which level-up path originally granted the box, so odds can't drift by source) rolls the outcome table in `config/tuning.json → mystery_outcomes`. At the shipped weights (as of the September 2026 `cats`/`coins_and_cats`/`buff` addition): a **5% pre-roll** first (a *Double Mystery* — two outcomes; nested rolls skip the pre-roll, so no chains), then per roll: **pack 52%** (the classic 1/totalvalue pull — plain single pack overall = 49.4%), **coins 10%** (500–2,500), **cats 10%** (5–15 cats rolled via `_spawn_weighted_cattype()`, the same season-gated spawn distribution as a live catch; new rarities are recorded to `discovered_cats` inline since the button path can't await `mark_discovered`), **coins_and_cats 5%** (500/750/1000 coins, weighted 60/25/15, plus 3–8 cats — a smaller bundle of both), **rain time 7%** (15s/30s/60s, banked in `profile.rain_seconds`, auto-rolls into a real bonus rain minute at 60s), **XP 6%** (250/500/1000), **scratchcard 4%**, **buff 4%** (a 24-hour `/jobs` perk drawn from `double_cat` 50% / `pack_drop_boost` 17% / `catch_xp_boost` 17% / `rarity_bump` 16% of the buff pool, granted via `_perks_grant(..., tier=3, duration_override=86400)` — fixes the duration at a full day regardless of the perk's normal tier-3 length; see [jobs.md → Tier scaling](jobs.md#tier-scaling)), **voucher 2%** (Double Pack 1.2% / Bounty Skip 0.64% / eGirl Bonus 0.16% ≈ 1-in-625 — the rarest thing in the box). EV ≈ 345 pack-value vs 292 for the old all-pack Mystery — the sweetener is carried entirely by rare outcomes; "usually it's just a pack" is the protected experience.

> **STALE:** the EV comparison above ("EV ≈ 345 pack-value vs 292…") was computed against the pre-September-2026 weights (pack 72%, six families, no `cats`/`coins_and_cats`/`buff`) and hasn't been recalculated for the current nine-family table — needs a fresh EV pass with agreed pack-value equivalents for `cats`, `coins_and_cats`, and `buff` (re: main.py, config/tuning.json, commit 4788e27).

**Vouchers 🎟️** are one-shot effects on `profile.vouchers` (JSONB, migration 035): *Double Pack* doubles the full contents (cats + coin variant) of the next pack opened (first pack of a multi-open batch — Open All or, as of the per-type quantity picker, a per-type Open N — matching the charge-perk precedent); *eGirl Bonus* forces the next `/catslots` spin to trigger the tier-3 eGirl Party (rides the same grid-overwrite mechanism as the admin force command; consumption is persisted before the spin animation); *Bounty Skip* autocompletes the first incomplete catnip bounty on the holder's next catch, with the normal completion toast/counter/achievements. Held vouchers render in `/battlepass`; they wipe at season rollover (pack-adjacent value) while the rain-seconds bank is preserved (it's rain, and rain survives the wipe).

**Three invariants for tuners:** (1) the max XP tier — doubled — must stay below the cheapest Mystery-bearing level cost (2,500 XP at S2+ L31), or an XP outcome could chain levels faster than it costs them (this bound was sized when Mystery resolved instantly inside the level loop; it's now vestigial for the *grant* path since grant time no longer resolves a Mystery at all, but still bounds `open_mystery`'s post-open `grant_achievement_xp()` call, which can itself chain further levels); (2) `resolve_mystery()` itself must never call `grant_achievement_xp()` or `progress()` directly — it only returns `bonus_xp` to its caller (see the docstring). The one caller today, the `/packs` "Open Mystery" button, runs **outside** any level loop, so it safely routes `bonus_xp` through `grant_achievement_xp()`. If `resolve_mystery()` is ever invoked again from *inside* a level loop (as it was pre-July-2026, at grant time), that caller must fold the XP into its own **local** accumulator instead — re-entering `grant_achievement_xp`/`progress` from inside their own level loop would double-count levels; (3) `"Mystery"` is valid only as a level's **primary** reward — the `extra_reward` stack has no Mystery branch and would crash on it.
...
**Slot composition (3/4 real vote, 1/4 substitute).** When `generate_quest("vote")` runs, it rolls `random.randint(1, 4)`:
- **Substitute roll (== 1, ~25%):** a single-action misc quest is picked at random from a filtered subset of `quests.misc` (see substitute pool rules below) and stored in `user.vote_quest` (the `profile.vote_quest` text column, default `''`). The substitute's XP range comes from the misc quest's own `xp_min/xp_max`. XP and claim timestamp still ride on `vote_reward` and `vote_cooldown` — no extra columns needed.
- **Otherwise (~75%):** the slot is set to the real "Vote on Top.gg" quest, drawn from `quests.vote` in `battlepass.json`. XP range is **300–450 XP base** with a **2× weekend multiplier**. The reward is randomized and stored in `user.vote_reward`.

If the substitute pool is empty (extreme config drift) or the pre-migration-028 `vote_quest` column is absent, the slot falls back to the real vote quest — real-vote share can only be pushed *above* 75%, never below.
...
**Catch-message vote button.** Appears when `VOTING_ENABLED` is true, conditioned in two stages: first `random.randint(1, 15) == 1` (roughly 1-in-15 catches, bumped up from 1-in-40 in the July 2026 visibility pass), then `vote_time_topgg + 43200 < now` (player is eligible to vote). Both must hold; the 12h eligibility gate is what keeps regular voters from seeing it often, since the random gate alone now fires fairly frequently.
...
**Design intent:** voting is now more enticing — the real-vote XP (300–450 base) is competitive with other daily quest rewards (not the smallest), reflecting an intent to make voting feel genuinely worthwhile rather than just a token bonus. The substitute mechanic still preserves a 5th XP source on no-vote days so the slot never sits dead: on non-vote cycles players receive an activity-driven misc quest instead. The substitute is constrained to single-action misc quests so it can complete naturally as a side effect of normal play, not as an extra obligation. The split has moved twice in the same direction — 1/3 real / 2/3 substitute, then 50/50, and now **75/25 real/substitute** — so the real vote quest now lands on about three of every four daily rerolls instead of every other one, continuing the push toward making the vote slot the default rather than the exception. The `refresh_quests` last-ordering rule ensures no quest appears in two slots at once, preserving the economy design of five distinct daily tasks.
...
**Scratchcard sources (as of the July 2026 visibility pass).** Weekly completion is no longer the only source: `"Scratchcard"` is now a first-class **battlepass level reward** type (`user.scratchcards += amount`, same claim code path as cats/packs/Mystery in both `grant_achievement_xp()` and `progress()`) — Season 4 levels 20 and 35 currently grant one this way instead of a Gold or Mystery pack. Level 30 of every season from Season 6 onward (originally a Season 5-only bonus, made standing in September 2026) additionally stacks an `extra_reward: "Scratchcard"` (`extra_amount: 1`) onto that level's 2-minute Rain reward, rather than replacing it. Scratchcards can also fall out of a Mystery box's outcome table (`scratchcard` family, 4% as of the September 2026 `cats`/`coins_and_cats`/`buff` rework, up from 1.5% — see [What a Mystery resolves to](#what-a-mystery-resolves-to)). Every source still funnels into the same `profile.scratchcards` counter and wipes at season rollover with the other pack-adjacent inventory, so the card economy still can't inflate across a reset — it just has more taps feeding it than the original "weekly quest only" design.

**Visibility.** Scratch cards are easy to lose track of, so grants get dedicated call-outs rather than blending into ordinary level-up lines: a bold inline reward line plus a follow-up splash (`_scratch_card_banner()` for the content-message splash `progress()` sends below a catch, `_scratch_card_banner_embed()` for the embed `grant_achievement_xp()` returns to its caller), each naming the running unscratched count. `/packs` also surfaces a display-only "🍀 Scratchcards (N)" button — it never opens (scratch cards aren't packs and are excluded from "Open all," which only sums `pack_data`) and clicking it just points the player at `/scratch`, where cards are actually revealed. The catch-message hint pool carries two dedicated `/scratch` reminder hints for players sitting on unrevealed cards.
