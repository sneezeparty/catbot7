...
## Unrepresented mechanics

> **STALE:** new mechanic `/jobs` send-screen crew-builder (from `main.py`) is not represented in design docs. The July 2026 rework replaced the free-text Add/Remove modals with on-message controls: a focus-rarity `Select`, `−5/−1/+1/+5/+Max` steppers, and three one-click Auto-fill buttons whose labels show their target success % (`JOBS_AUTOFILL_TARGETS = {"good": 0.70, "lock": 0.90, "max": <sigmoid ceiling>}`). Success % now recomputes on every click, with a "+1 → X% · +Max → Y%" projection for the focused rarity.
>
> The Auto-fill algorithm (`_jobs_auto_fill_crew()`) is a value-weighted spread: it inverts the [success sigmoid](#the-two-die-structure) to find the crew muscle needed to hit the target %, then water-fills by marginal-muscle-per-coin-value (binary-searching a single efficiency threshold `lam`) so the crew draws from the player's cheapest rarities first and only reaches into pricier ones once cheap supply is exhausted — replacing an earlier commons-first bisection that could dump ~1,000 of one common type into a single crew. Every Auto-fill target reserves ≥1 of every owned rarity (so prism-crafting stays possible) and is best-effort: if spare inventory can't reach the target, it commits all available spare and reports the shortfall. A manual `+Max` on the stepper row does **not** carry the ≥1 reserve — only the Auto-fill buttons protect it.
>
> No design doc currently states *why* Auto-fill should minimize cat-value-at-risk rather than crew *count*, or why the ≥1-per-rarity reserve matters (crew cats are lost outright on near-miss/total-failure, so cheap commons are deliberately spent first and rares are protected both from being risked and from being fully depleted out of the prism-crafting supply). The rationale is currently only written down in the `_jobs_auto_fill_crew()` docstring and commit messages — a human should fold it into this doc, most likely as a subsection under [The two-die structure](#the-two-die-structure) or [Reputation's dual role](#reputations-dual-role) once the crew-building UI has a natural home here.
