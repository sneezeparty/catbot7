...
## Crew-building & Auto-fill

The `/jobs` send screen builds a crew entirely on the message (no modals): a focus-rarity `Select`, `−5/−1/+1/+5/+Max` steppers on the focused rarity, and three one-click **Auto-fill** buttons labeled by their target success chance (`JOBS_AUTOFILL_TARGETS = {"good": 0.70, "lock": 0.90, "max": <sigmoid ceiling>}`). The success % recomputes on every click, with a "+1 → X% · +Max → Y%" projection for the focused rarity.

**How Auto-fill picks a crew.** `_jobs_auto_fill_crew()` is a value-weighted spread. It inverts the [success sigmoid](#the-two-die-structure) to find the crew muscle needed to hit the target %, then water-fills by marginal-muscle-per-coin-value (binary-searching a single efficiency threshold `lam`), so the crew draws from the player's cheapest rarities first and only reaches into pricier ones once cheap supply is exhausted. Every target reserves ≥1 of every owned rarity and is best-effort: if spare inventory can't reach the target, it commits all available spare and reports the shortfall.

**Why value-at-risk, not crew count.** Crew cats are lost outright on near-miss (half, rounded up) and total-failure (all) — see [The two-die structure](#the-two-die-structure). So Auto-fill deliberately minimizes the *coin value* the player stands to lose, not the *number* of cats sent: it spends cheap commons first and leaves rares untouched until forced. Because each rarity's send power has diminishing returns (`sp · n^0.75`), spreading across several cheap tiers is also *more* muscle-efficient than piling one type — so the value-first policy and the anti-pile behavior fall out of the same optimization. (An earlier commons-first bisection could dump ~1,000 of a single common type into one crew; the spread replaced it.)

**Why the ≥1-per-rarity reserve.** Prism-crafting consumes one of *every* cat type, so an Auto-fill that emptied a rarity would silently break the player's ability to craft prisms. Reserving one of each protects rares twice over — from being risked *and* from being drained out of the prism supply. The reserve is an Auto-fill guarantee only: a manual `+Max` on the stepper row can still send everything of the focused rarity, on the principle that an explicit player action overrides the safety net.
