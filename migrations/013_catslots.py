...
"""Add /catslots state columns.

Five counter columns to track lifetime activity in the new /catslots casino
game, and four boolean ach columns matching the new aches in config/aches.json.

Idempotent (per-column gated). Bot MUST be stopped before running. Run with
the same env vars as bot.py:

    TOKEN=... psql_password=... python migrations/013_catslots.py
"""
...
MARKER = REPO_ROOT / "migrations" / "013.done"
LOGFILE = REPO_ROOT / "migrations" / "013.log"


# (table, column, type, default, not_null)
COLUMNS: list[tuple[str, str, str, str, bool]] = [
    ("profile", "catslots_spins", "integer", "0", True),
    ("profile", "catslots_wins", "integer", "0", True),
    ("profile", "catslots_big_wins", "integer", "0", True),
    ("profile", "catslots_coins_bet", "bigint", "0", True),
    ("profile", "catslots_coins_won", "bigint", "0", True),
    ("profile", "catslots", "boolean", "false", True),
    ("profile", "win_catslots", "boolean", "false", True),
    ("profile", "big_win_catslots", "boolean", "false", True),
    ("profile", "paradoxical_catslots", "boolean", "false", True),
]
...
async def main() -> int:
    ...
    log("starting migration 013_catslots")
