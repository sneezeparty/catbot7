...
"""Add the JSONB column that tracks which catstore pack tiers a player has
bought at least once. Backs the new `catstore_pack_collector` achievement,
parallel to `store_purchased_rarities` on the cat side.

Adds one column:

  - profile.store_purchased_pack_tiers JSONB NOT NULL DEFAULT '[]'::jsonb

Existing profiles get [] — no backfill needed.

Idempotent (per-column gated). Bot MUST be stopped before running. Run with
the same env vars as bot.py:

    TOKEN=... psql_password=... python migrations/015_store_pack_tiers.py
"""
...
MARKER = REPO_ROOT / "migrations" / "015.done"
LOGFILE = REPO_ROOT / "migrations" / "015.log"


# (table, column, type, default, not_null)
COLUMNS: list[tuple[str, str, str, str, bool]] = [
    ("profile", "store_purchased_pack_tiers", "jsonb", "'[]'::jsonb", True),
]
...
async def main() -> int:
    ...
    log("starting migration 015_store_pack_tiers")
