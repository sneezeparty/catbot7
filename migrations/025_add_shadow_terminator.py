...
"""Per-rarity catch counters for the two new cats (Shadow, Terminator).

Adds two columns to the profile table:

  - profile."cat_Shadow"     INTEGER DEFAULT 0  -- per-server Shadow catches (5.1%)
  - profile."cat_Terminator" INTEGER DEFAULT 0  -- per-server Terminator catches (0.115%)

These mirror the existing per-rarity `cat_<Rarity>` columns (Fine, Nice, ...,
eGirl) populated by the catch handler and the jobs send/return paths.

Shadow lives at weight 221 (between Baby and Epic); Terminator at weight 5
(tied with Real). Both are season-gated via `rarity_min_season` in tuning.json
(currently min_season=2, the current season, so they're live immediately).
...
the same env vars as bot.py:
    ...
    TOKEN=... psql_password=... python migrations/025_add_shadow_terminator.py
"""
...
MARKER = REPO_ROOT / "migrations" / "025.done"
LOGFILE = REPO_ROOT / "migrations" / "025.log"


# (table, column, type, default, not_null)
COLUMNS: list[tuple[str, str, str, str, bool]] = [
    ("profile", "cat_Shadow", "integer", "0", False),
    ("profile", "cat_Terminator", "integer", "0", False),
]
...
async def main() -> int:
    ...
    log("starting migration 025_add_shadow_terminator")
